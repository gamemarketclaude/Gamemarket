...
{"blocked": true или false, "reason": "краткая причина на русском, одно предложение"}`;
  ...
  try {
    ...
    if (!response.ok) {
      console.error('[aiModeration] API вернул ошибку:', response.status, await response.text());
      return null;
    }
    ...
    if (!textBlock) {
      console.error('[aiModeration] В ответе нет текстового блока:', JSON.stringify(data));
      return null;
    }
