...
- `ADMIN_EMAILS` — ваша почта (или несколько через запятую): после входа под ней появится «Админка» с баном по почте
...
объявления, пользователи и заказы сохраняются. Исключение — обновление, где
меняется устройство базы (например, переход со входа по логину на вход по
почте): тогда старая база автоматически сохраняется как
`db/database.backup-...sqlite`, а сайт создаёт новую.

Забанить пользователя с сервера (если админка не настроена):
